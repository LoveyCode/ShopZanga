...
let newCollection = products.slice(1).slice(-15)
...
app.post('/addtowishlist', fetchUser, async (req, res)=>{
    ...
    await Users.findOneAndUpdate({_id:req.user.id}, {cartData:userData.wishlistData})
    res.send('Added')
})
...
app.post('/removefromwishlist', fetchUser,async(req, res)=>{
    ...
    userData.wishlistData[req.body.itemId] -=1;
    await Users.findOneAndUpdate({_id:req.user.id}, {wishData:userData.wishlistData})
    res.send('Removed')
})
